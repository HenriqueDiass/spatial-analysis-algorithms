# discovery/inspectors/cnes_inspector.py

import json
from pysus.ftp.databases import CNES
from typing import Dict, Any, Optional

def get_cnes_parameters() -> Optional[Dict[str, Any]]:
    """
    Exibe os grupos do CNES em formato JSON, coleta os parâmetros do usuário de forma interativa,
    valida os dados e os retorna em um dicionário.
    """
    print("--- 🔬 Coleta de Parâmetros para o CNES ---")
    try:
        cnes = CNES().load()
        ...
        print("\n📋 Grupos de dados disponíveis no CNES:")

        groups_list = [
            {"code": code, "name": description} for code, description in cnes.groups.items()
        ]
        ...
        group_code = input("\nDigite o código do grupo (ex: ST): ").strip().upper()
        ...
        # --- Valida as entradas ---
        if group_code not in cnes.groups:
        ...
        print(f"❌ Erro ao buscar opções do CNES: {e}")
        return None