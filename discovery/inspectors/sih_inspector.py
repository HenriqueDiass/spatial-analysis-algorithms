# discovery/inspectors/sih_inspector.py

import json
from pysus.ftp.databases import SIH
from typing import Dict, Any, Optional

def get_sih_parameters() -> Optional[Dict[str, Any]]:
    """
    Exibe os grupos do SIH em formato JSON, coleta os parâmetros do usuário de forma interativa,
    valida os dados e os retorna em um dicionário.
    """
    print("--- 🔬 Coleta de Parâmetros para o SIH ---")
    try:
        sih = SIH().load()
        ...
        print("\n📋 Grupos de dados disponíveis no SIH:")

        groups_list = [
            {"code": code, "name": description} for code, description in sih.groups.items()
        ]
        ...
        group_code = input("\nDigite o código do grupo (ex: RD): ").strip().upper()
        ...
        # --- Valida as entradas ---
        if group_code not in sih.groups:
        ...
        print(f"❌ Erro ao buscar opções do SIH: {e}")
        return None