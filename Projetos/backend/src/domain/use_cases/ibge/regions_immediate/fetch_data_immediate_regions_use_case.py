# src/domain/use-cases/ibge/regions_immediate/fetch-data-immediate-regions.use-case.py
"""
Use case to fetch geometry data for all Immediate Regions in Brazil.
"""
...
IBGE_REGION_TYPE_IMMEDIATE = "regioes-imediatas"
API_REQUEST_DELAY_SECONDS = 0.1

class FetchDataImmediateRegionsUseCase:
    """
    Orchestrates fetching data from the IBGE API for all Immediate Regions
    ...
    def execute(self) -> Optional[List[Dict[str, Any]]]:
        ...
        Returns:
            A list of GeoJSON features with the data for all immediate regions,
        ...
        # Step 2: Loop through each state to find its immediate regions.
        for _, state_row in states_dataframe.iterrows():
            state_id = state_row['id']
            state_abbreviation = state_row['abbreviation']

            # Step 3: For the current state, fetch its list of immediate regions.
            regions_dataframe = ibge_client.fetch_regions_by_state(state_id, IBGE_REGION_TYPE_IMMEDIATE)
            ...
            for _, region_row in regions_dataframe.iterrows():
                ...
                geographic_mesh = ibge_client.fetch_geojson_mesh("regioes-imediatas", region_id)
                ...
                if geographic_mesh and 'features' in geographic_mesh and geographic_mesh['features']:
                    ...
                    single_feature['properties']['immediate_region_id'] = region_id
                    single_feature['properties']['immediate_region_name'] = region_name
